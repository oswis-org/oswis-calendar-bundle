...
class WebAdminParticipantsListController extends AbstractController
{
    ...
    // ── Filtrování, hledání a řazení vlastní sdílená služba {@see ParticipantListFilter} ──
    // Obálky drží volací místa beze změny. Jediný vlastník logiky je služba, protože tutéž
    // otázku potřebuje i export — a když ji měl vlastní, rozešel se s obrazovkou (2 vs. 309).

    /** @return list<array{key: string, label: string, group: string, expr: string|null}> */
    private function getFilterRegistry(): array
    {
        return $this->listFilter->getFilterRegistry();
    }

    private function isKnownFilter(string $key): bool
    {
        return $this->listFilter->isKnownFilter($key);
    }
     ...
     */
    private function compileFilterExpression(string $filterKey, array $selectedFlags, ?string $advancedExpr, array $slugToCategory, bool $excludeDeleted = true): string
    {
        return $this->listFilter->compileFilterExpression($filterKey, $selectedFlags, $advancedExpr, $slugToCategory, $excludeDeleted);
    }
    ...
    {
        return $this->listFilter->buildFlagOffering($selectedFlags);
    }

    private function normalizeSort(string $sort): string
    {
        return $this->listFilter->normalizeSort($sort);
    }
     ...
     */
    private function applyListFilters(Collection $loaded, ?string $expression, ?string $q, string $sort, string $dir): array
    {
        return $this->listFilter->apply($loaded, $expression, $q, $sort, $dir);
    }
