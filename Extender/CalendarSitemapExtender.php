...
class CalendarSitemapExtender implements SiteMapExtenderInterface
{
    ...
    public function getItems(): Collection
    {
        ...
        $items = $itemsData->map(
            fn(array $data) => new SiteMapItem($data['path'], $data['changeFrequency']),
        );
        foreach ($this->eventService->getRepository()->getEvents([EventRepository::CRITERIA_ONLY_PUBLIC_ON_WEB => true]) as $event) {
            ...
            try {
                $items->add(
                    new SiteMapItem(
                        $this->urlGenerator->generate('oswis_org_oswis_calendar_web_event', ['eventSlug' => $event->getSlug()]),
                        null,
                        $event->getUpdatedAt()
                    )
                );
                $items->add(
                    new SiteMapItem(
                        $this->urlGenerator->generate('oswis_org_oswis_calendar_web_event_leaflet', ['eventSlug' => $event->getSlug()]),
                        null,
                        $event->getUpdatedAt()
                    )
                );
